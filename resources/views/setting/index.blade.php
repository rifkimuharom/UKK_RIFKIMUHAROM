...
@section('content')
    {{-- CDN Tailwind (opsional jika layout utama belum memasang Tailwind) --}}
    ...
    <form action="{{ route('setting.update') }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="p-6 bg-slate-50 min-h-screen font-sans space-y-6">
            ...
            @if(session('success'))
                <div
                    class="p-4 bg-green-100 border border-green-300 text-green-800 rounded-xl flex justify-between items-center text-sm font-semibold">
            ...
            <div class="grid grid-cols-1 xl:grid-cols-12 gap-6">
                ...
                <div class="xl:col-span-6 bg-white p-6 rounded-2xl border border-gray-100 shadow-sm">
                        ...
                        <div class="space-y-4">
                            <div>
                                ...
                                <input type="text" name="nama_toko"
                                    value="{{ old('nama_toko', $setting->nama_toko ?? 'KUDE POS') }}"
                            ...
                            <div>
                                ...
                                <textarea name="alamat_toko" rows="3"
                                    class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">{{ old('alamat_toko', $setting->alamat_toko ?? 'Jl. Raya Utama No. 123, Bandung') }}</textarea>
                            </div>
                            <div>
                                <label class="block text-xs font-semibold text-gray-700 mb-1">Nomor Telepon /
                                    WhatsApp</label>
                                <input type="text" name="telepon_toko"
                                    value="{{ old('telepon_toko', $setting->telepon_toko ?? '0812-3456-7890') }}"
                                    class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
                            </div>
                        </div>
                    </div>
                    ...
                    <div id="content-logos" class="tab-content hidden space-y-4">
                        ...
                        <label
                            class="block border-2 border-dashed border-gray-300 rounded-xl p-8 text-center cursor-pointer hover:border-blue-500 transition bg-slate-50">
                            <p class="text-sm font-semibold text-gray-600" id="dropzone-text">Klik untuk upload Logo Toko
                                Baru</p>
                    ...
                    <div id="content-struk" class="tab-content hidden space-y-4">
                        ...
                        <div class="space-y-4">
                            <div>
                                <label class="block text-xs font-semibold text-gray-700 mb-1">Pajak Pertambahan Nilai / PPN
                                    (%)</label>
                                <input type="number" step="0.1" name="pajak_persen"
                                    value="{{ old('pajak_persen', $setting->pajak_persen ?? 10) }}"
                                    class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
                            </div>
                            <div>
                                <label class="block text-xs font-semibold text-gray-700 mb-1">Header Struk</label>
                                <input type="text" name="header_struk"
                                    value="{{ old('header_struk', $setting->header_struk ?? 'Selamat Datang Di KUDE POS') }}"
                                    class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
                            </div>
                            <div>
                                <label class="block text-xs font-semibold text-gray-700 mb-1">Footer Struk</label>
                                <input type="text" name="footer_struk"
                                    value="{{ old('footer_struk', $setting->footer_struk ?? 'Terima kasih atas kunjungan Anda') }}"
                                    class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
                            </div>
                        </div>
                    </div>
                ...
                <div class="xl:col-span-3 bg-white p-5 rounded-2xl border border-gray-100 shadow-sm h-fit space-y-4">
                    ...
                    <div class="bg-slate-100 p-3 rounded-xl">
                        <div
                            class="bg-white p-4 rounded-t-sm shadow font-mono text-[11px] text-gray-700 space-y-2 border-t-4 border-blue-600">
                            <div class="text-center space-y-0.5">
                                <p class="font-bold text-xs uppercase text-gray-900" id="preview-nama-toko">
                                    {{ $setting->nama_toko ?? 'KUDE POS' }}</p>
                                <p class="text-[9px] text-gray-500 leading-tight" id="preview-alamat">
                                    {{ $setting->alamat_toko ?? 'Jl. Raya Utama No. 123, Bandung' }}</p>
                                <p class="text-[9px] text-gray-500" id="preview-telepon">Telp:
                                    {{ $setting->telepon_toko ?? '0812-3456-7890' }}</p>
                                <div class="border-b border-dashed border-gray-300 my-1"></div>
                                <p class="font-semibold text-[10px]" id="preview-header">
                                    {{ $setting->header_struk ?? 'Selamat Datang Di KUDE POS' }}</p>
                            </div>
                            ...
                            <div class="space-y-1 text-[10px]">
                                ...
                                <div class="flex justify-between text-gray-500">
                                    <span id="preview-pajak-label">Pajak ({{ $setting->pajak_persen ?? 10 }}%)</span>
                                    <span id="preview-pajak-val">3.300</span>
                                </div>
                                <div
                                    class="flex justify-between font-bold text-xs text-gray-900 pt-1 border-t border-gray-200">
                                    <span>TOTAL</span>
                                    <span id="preview-total">36.300</span>
                                </div>
                            </div>
                            ...
                            <div class="text-center pt-1">
                                <p class="text-[10px] italic text-gray-600" id="preview-footer">
                                    {{ $setting->footer_struk ?? 'Terima kasih atas kunjungan Anda' }}</p>
                            </div>
                        </div>
                    </div>
                </div>
    ...
    <script>
        ...
        const map = {
            ...
            'alamat_toko': 'preview-alamat',
            'header_struk': 'preview-header',
        ...
        const telpInput = document.querySelector('[name="telepon_toko"]');
        if (telpInput) {
        ...
        const pajakInput = document.querySelector('[name="pajak_persen"]');
        if (pajakInput) {
            pajakInput.addEventListener('input', (e) => {
